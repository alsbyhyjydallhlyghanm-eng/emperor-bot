...
def send_welcome(message):
    ...
    btn11 = telebot.types.InlineKeyboardButton("🛡️ الإدارة العليا والمطور 🏛", callback_data="admin")
...
def callback_query(call):
    ...
    # --- زر الإدارة العليا والمطور ---
        ...
        admin_text = (
            f"🏛 **الإدارة العليا والمطور** 👑\n\n"
            f"👤 المطور الأساسي: **جيدالله المحولي**\n"
            f"💬 معرف التواصل: @{DEVELOPER_USERNAME}\n\n"
            f"☑️ بوت الإمبراطور للخدمات الرقمية يعمل بكفاءة تامة."
        )
        m = telebot.types.InlineKeyboardMarkup()
        m.add(
            telebot.types.InlineKeyboardButton("🌐 مراسلة المطور مباشرة", url=f"https://t.me/{DEVELOPER_USERNAME}"),
            telebot.types.InlineKeyboardButton("🔙 - العودة للقائمة", callback_data="back_home")
        )
        try:
            bot.edit_message_text(admin_text, call.message.chat.id, call.message.message_id, reply_markup=m, parse_mode="Markdown")
        except:
            bot.send_message(call.message.chat.id, admin_text, reply_markup=m, parse_mode="Markdown")
